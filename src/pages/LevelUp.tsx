...
const LevelUp = () => {
  const [levelUpPosts, setLevelUpPosts] = useState<Post[]>([]);
  const [loading, setLoading] = useState(true);

  const sectionConfig = navigationConfig.sections.levelUp;

  useEffect(() => {
    const timer = setTimeout(() => {
      const filteredPosts = posts.filter((post: Post) => post.category === sectionConfig.category);
      setLevelUpPosts(filteredPosts);
      setLoading(false);
    }, 1000);
    ...
    <>
      ...
      <div className="container mx-auto py-8 md:py-12 lg:py-16 px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-12 gap-4 md:gap-6 lg:gap-8">
          ...
          ) : levelUpPosts.length > 0 ? (
            levelUpPosts.map((post) => (
