...
const Cases = () => {
  const { posts: casePosts, loading, error } = usePostsByCategory('Case Study');
      ...
      <div className="container mx-auto py-8 md:py-12 lg:py-16 px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-12 gap-4 md:gap-6 lg:gap-8">
          {showSkeleton ? (
            ...
            <p className="col-span-12 text-red-500 text-base sm:text-lg">Error loading cases: {error}</p>
          ) : casePosts.length > 0 ? (
            casePosts.map((post) => (
              <PostCard 
                key={post.id} 
